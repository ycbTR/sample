  ...
  def self.mass_assign(file)
    ...
    loop do
      ...
      if current_row[0].nil?
        current_lot = LotNumber.create(number: LotNumber.maximum("number") + 1, region: current_row[4], provenance: current_row[5], location: current_row[6], spa_name: nil, spa_specific: true, self_heritage: false, mass_num: mass_number)
      else
        current_lot = LotNumber.create(number: current_row[0], region: current_row[4], provenance: current_row[5], location: current_row[6], spa_name: nil, spa_specific: true, self_heritage: false, mass_num: mass_number)
      end
